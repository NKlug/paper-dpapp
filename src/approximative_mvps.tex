 Nach dem wir jetzt einige Vorarbeit geleistet haben, werden wir in diesem Kapitel sehen, wie man das MVPS-Problem mit Hilfe einer WSPD bis auf ein $\epsilon$ genau approximieren kann - und dass (für festes $t$ und $\epsilon$) in $O(n\log n)$ Zeit.
    ...
    Diese erhalten wir, indem wir für alle $1 \leq i \leq n$ $x_i = \delta(p_1, p_i)$ setzen.
    ...
    \begin{lemma}
    	...
    	Ist weiter $1 \leq t < \frac{s^2}{4s + 16}$ und ist das Tupel $(p, q)$ $t$-distanzerhaltend, dann ist $(p', q')$ $t'$-distanzerhaltend, wobei $t'$ gegeben ist durch 
    	\[
    ...
    Durch diese Wahl von $s$ ist die Einschränkung $t < \frac{s^2}{4s + 16}$ von Lemma \ref{lem:stranget} für alle $t \geq 1$ erfüllt.
    ...
    Dass diese Breitensuche einen kürzesten Pfad in $H$ zurückgibt, ist nicht sofort klar. 
