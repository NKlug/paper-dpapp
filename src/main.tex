...
\begin{document}
    ...
    Der Algorithmus besteht aus vier Teilen, von denen wir die ersten drei schon betrachtet haben.
    ...
    \begin{enumerate}
    	...
    	%\item Führe die unten aufgeführte modifizierte Breitensuche in $T$ durch, um einen kürzesten Weg von einer Menge $A_{i_1}$, die $x_1$ enthält, zu einer Menge $B_{i_{k-1}}$, die $x_n$ enthält, zu bestimmen.
    \end{enumerate}
    ...
    \end{itemize}
    Nehmen wir an, dass die Knoten von $T$ mit den Zahlen $1, 2, ... n'$ benannt sind, können wir $color$, $dist$ und $parent$ zum Beispiel durch drei Arrays der Größe $n'$ realisieren.
    ...
    \begin{description}
    	...
    	\begin{description}
    		...
    		\item[\textbf{Schritt 3.2:}] Starte bei $v_i$ und besuche alle Knoten des Teilbaums von $T$, dessen Wuzel $v_i$ ist. Für alle besichtigten Knoten $k'$, tue Folgendes:
    		
    		Setze $color[k'] = gray$. Falls $k'$ ein A-Knoten ist, setze $dist[k'] = dist[k] + 1$, $parent[k']	= k$ und füge $k'$ in $W$ ein.
    	\end{description}
    	
    	\item[\textbf{Schritt 4:}] Bereche den Pfad $Z = (z, parent[z], parent[parent[z]], parent^3[z],  \mathellipsis, parent^{k-1}[z])$, wobei $k = dist[z] + 1$.
    	
    	\item[\textbf{Schritt 5:}] Konvertiere den umgekehrten Pfad $Z$, der den Kriterien von Satz \ref{theo:H2Approx} entspricht, wie im Beweis des selben Satzes zu einer Approximation von $P$.
    \end{description}
