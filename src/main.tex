...
\begin{document}
    ...
    Sei wieder $d\geq1$, $t>1$ und $P = (p_1, p_2, \mathellipsis, p_n)$ ein Pfad in $\R^d$. Sei weiter $P^*$ die Menge der t-distanzerhaltende Approximationen mit der geringsten Knotenzahl.
    ...
    Nun betrachten wir noch die Laufzeit: Die Konstruktion von $G_t$ gelingt uns in $O(n^2)$, da wir für maximal $n^2$ Kanten überprüfen müssen, ob diese t-distanzerhaltend sind. Aus  \cite{hagerup}

    - Exakter Algorithmuns MVPS: Satz, Beweis\n
    - Exakter Algorithmus MDPS: Satz, Beweis


    \section{Heuristische Algorithmen}
    \label{sec:heuristic}
